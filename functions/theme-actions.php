...
<div class="top">
<?php if ( $mts_options['mts_footer_nav'] == '1') { ?>
...
function mts_the_category( $separator = ', ' ) {
    ...
    foreach ( $categories as $i => $category ) {
        echo '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . sprintf( __( "%s : kategorisindeki yazıları göster", 'sociallyviral' ), esc_attr( $category->name ) ) . '">' . esc_html( $category->name ).'</a>';
        if ( $i < $count - 1 )
            echo $separator;
    }
}
...
function mts_the_tags($before = '', $sep = ', ', $after = '') {
    $before = '<div class="tags border-bottom">'.__('Tags: ', 'sociallyviral' );
    $after = '</div>';
    ...
    function mts_pagination($pages = '', $range = 3) {
        ...
        if (isset($mts_options['mts_pagenavigation_type']) && $mts_options['mts_pagenavigation_type'] == '1' ) { // numeric pagination
            the_posts_pagination( array(
                'mid_size' => 5,
                'prev_text' => __( 'Önceki', 'sociallyviral' ),
                'next_text' => __( 'Sonraki', 'sociallyviral' ),
            ) );
        } else { // traditional or ajax pagination
            ?>
            <div class="pagination pagination-previous-next">
            <ul>
                <li class="nav-previous"><?php next_posts_link( '<i class="fa fa-angle-left"></i> '. __( 'Önceki', 'sociallyviral' ) ); ?></li>
                <li class="nav-next"><?php previous_posts_link( __( 'Sonraki', 'sociallyviral' ).' <i class="fa fa-angle-right"></i>' ); ?></li>
            </ul>
            </div>
            <?php
        }
